...
def get_obj_data(obj, name):
    ...
    getvertex = '(v\\s+)'+floating+'\\s+'+floating+'\\s+'+floating+'\\s+'+floating+'\\s+'+floating+'\\s+'+floating
    ...
    for line in infile:
        if line[0]=='#':                    #we have a comment line
            ...
            if m:                               #we have a valid vertex
                ...
                obj.col.append( [255*float(m.group(5)), 255*float(m.group(6)), 255*float(m.group(7)) ] )
...
def mix_colours(r0, g0, b0, r1, g1, b1, r2, g2, b2):
    r = 0.33*(r0+r1+r2)
    g = 0.33*(g0+g1+g2)
    b = 0.33*(b0+b1+b2)
...
def draw_faces( faces_data, pts, obj, shading, fill_col,st, parent):          
    for face in faces_data:#for every polygon that has been sorted
        ...
        r0,g0,b0 = obj.col[ obj.fce[face_no][0]-1 ]
        r1,g1,b1 = obj.col[ obj.fce[face_no][1]-1 ]
        r2,g2,b2 = obj.col[ obj.fce[face_no][2]-1 ]
        col      = mix_colours( r0, g0, b0, r1, g1, b1, r2, g2, b2 )
        st.fill  = get_darkened_colour( col, 1.0 )
